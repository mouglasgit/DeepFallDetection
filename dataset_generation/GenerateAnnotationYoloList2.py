...
with tf.Session() as sess:
    ...
    idx = 0
    list_dirs = range(1, 25)
    size = len(list_dirs)
    for atual in list_dirs:
        path_abs = '/media/oberon/MouglasHD2T/actions_datasets/MultCamFallDataset/data/chute{:02d}'.format(atual)
        
        dir_imgs = '{}/JPEGImages/'.format(path_abs)
        dir_anno = '{}/Annotations/'.format(path_abs)
        
        if not os.path.exists(dir_imgs):
            os.makedirs(dir_imgs)
             
        if not os.path.exists(dir_anno):
            os.makedirs(dir_anno)
        
        cap = cv2.VideoCapture('{}/cam2.avi'.format(path_abs))
        
        id_img = 0
        count_f = 0
        while True:
            r, img_ori = cap.read()
            if r == False or img_ori is None:
                break
            
            if count_f >= 4:
                count_f = 0
                name_img = "{:06d}.jpg".format(id_img)
                cv2.imwrite("{}{}".format(dir_imgs, name_img), img_ori)
                ...
                if len(objetos_coo) > 0:
                    ge_file_xml(dir_anno, name_img, width_ori, height_ori, objetos_coo)
